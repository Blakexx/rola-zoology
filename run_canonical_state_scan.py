"""Runner for canonical_state_scan sweep (17 cells × 5 seeds = 85 runs)."""
...
RESULTS_FILE = ZOO / "canonical_state_scan_results.jsonl"


def load_configs_and_envs():
    spec = importlib.util.spec_from_file_location("css",
        ZOO / "zoology/experiments/canonical_state_scan.py")
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    return mod.configs, mod.configs_envs


def run_one(idx, run_id, cfg, env_overrides):
    tmp_path = Path(f"/tmp/_css_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_css_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_css_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
    ...
    try:
        ...
        return {"run_id": run_id, "idx": idx, "ok": False, "elapsed": 3600, "error": "timeout"}


def main():
    configs, envs = load_configs_and_envs()
    print(f"Loaded {len(configs)} canonical-state-scan configs", flush=True)
    completed = set()
