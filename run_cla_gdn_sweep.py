"""CLA-GDN config sweep runner. Picks up per-config env overrides from
cla_gdn_sweep.py's configs_envs list (parallel to configs).
"""
...
RESULTS_FILE = ZOO / "cla_gdn_sweep_results.jsonl"

def load_configs_and_envs():
    spec = importlib.util.spec_from_file_location("clags",
        ZOO / "zoology/experiments/cla_gdn_sweep.py")
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    return mod.configs, mod.configs_envs

def run_one(idx, run_id, cfg, env_overrides):
    tmp_path = Path(f"/tmp/_clags_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_clags_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_clags_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
    ...
    try:
        ...
        return {"run_id": run_id, "idx": idx, "ok": ok, "elapsed": elapsed,
                ...
                "env": env_overrides, "peakiness": peakiness,
...
def main():
    ...
    print(f"Loaded {len(configs)} cla-gdn-sweep configs", flush=True)
    completed = set()
