...
def gdn_matched(target):
    """GDN with head_dim=32 fixed; expand_v must be integer to avoid the
    float-value_dim bug in zoology's wrapper (line 118: value_dim = key_dim *
    expand_v, no int cast). expand_v ∈ {1, 2, 3, 4} → state ∈ {4096, 8192,
    12288, 16384}. Pick the nearest integer expand_v.
    """
    expand_v = max(1, round(target / 4096))
    head_v_dim = 32 * expand_v
    state = 4 * 32 * head_v_dim  # = 128 * 32 * expand_v
