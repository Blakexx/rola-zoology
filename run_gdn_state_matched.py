"""Runner for state-matched FLA-GDN baseline sweep (4 cells × 5 seeds = 20 runs).
Strips out any CLA-recipe env vars from the parent shell so baseline runs are clean.
"""
...
RESULTS_FILE = ZOO / "gdn_state_matched_results.jsonl"


def load_configs():
    spec = importlib.util.spec_from_file_location("gdn_sm",
        ZOO / "zoology/experiments/gdn_state_matched.py")
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    return mod.configs


def run_one(idx, run_id, cfg):
    tmp_path = Path(f"/tmp/_gdnsm_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_gdnsm_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_gdnsm_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
...
def main():
    ...
    print(f"Loaded {len(configs)} gdn-state-matched configs", flush=True)
    completed = set()
    ...
    with open(RESULTS_FILE, "a") as fout:
        for i, cfg in enumerate(configs):
            ...
            run_id = cfg.run_id
            print(f"\n[{i+1}/{len(configs)}] {run_id}", flush=True)
