"""Same as run_head_to_head.py but uses h2h_pos0.py and writes to h2h_pos0_results.jsonl."""
...
RESULTS_FILE = ZOO / "h2h_pos0_results.jsonl"

def load_configs():
    spec = importlib.util.spec_from_file_location("h2h_pos0",
        ZOO / "zoology/experiments/h2h_pos0.py")
...
def run_one(idx, run_id, cfg):
    tmp_path = Path(f"/tmp/_h2hp0_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_h2hp0_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_h2hp0_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
...
def main():
    ...
    print(f"Loaded {len(configs)} h2h-pos0 configs", flush=True)
    completed = set()
