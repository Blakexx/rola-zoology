"""Per-state ladder: the per-state endpoint at every rung, completing the normalization
comparison across the ladder (global-sym and kappa-asym ladders are banked; per-state
existed only at nc=256 + old normAB nc=16/64). Config otherwise identical to
rola_rla_ladder (same data, lr, seed, rank diagnostic)."""
import sys
...
for nc in NCS:
    kw = rola_instance("rola-rla-sym-ps", d_qk=12, d_v=12, num_chunks=nc, n_heads=4)
    ...
    configs.append(TrainConfig(
        ...
        logger=LoggerConfig(project_name="rola-ps-ladder", entity=""),
        max_epochs=40, learning_rate=LR, weight_decay=0.0, seed=SEED,
        run_id=f"ladder-nc{nc}-d12dv12_ps_sym_lr{LR:.0e}_s{SEED}",
