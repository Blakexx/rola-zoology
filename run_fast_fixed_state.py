"""Runner for fast_fixed_state sweep (14 cells × 5 seeds = 70 runs).
Applies per-config env overrides for the CLA recipe.
"""
...
RESULTS_FILE = ZOO / "fast_fixed_state_results.jsonl"


def load_configs_and_envs():
    spec = importlib.util.spec_from_file_location("ffs",
        ZOO / "zoology/experiments/fast_fixed_state.py")
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    return mod.configs, mod.configs_envs


def run_one(idx, run_id, cfg, env_overrides):
    tmp_path = Path(f"/tmp/_ffs_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_ffs_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_ffs_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
...
def main():
    ...
    print(f"Loaded {len(configs)} fast-fixed-state configs", flush=True)
    completed = set()
