"""Runner for the conv-on sweep (8 cells × 5 seeds = 40 runs).
Applies per-config env overrides for the CLA recipe (writer/reader stds + curriculum).
"""
...
RESULTS_FILE = ZOO / "conv_sweep_results.jsonl"


def load_configs_and_envs():
    spec = importlib.util.spec_from_file_location("cs",
        ZOO / "zoology/experiments/conv_sweep.py")
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    return mod.configs, mod.configs_envs


def run_one(idx, run_id, cfg, env_overrides):
    tmp_path = Path(f"/tmp/_cs_single_{idx}.py")
    tmp_path.write_text(
        "import pickle\n"
        f"with open('/tmp/_cs_cfg_{idx}.pkl', 'rb') as f:\n"
        "    configs = [pickle.load(f)]\n"
    )
    with open(f"/tmp/_cs_cfg_{idx}.pkl", "wb") as f:
        pickle.dump(cfg, f)
...
def main():
    ...
    print(f"Loaded {len(configs)} conv-sweep configs", flush=True)
    completed = set()
